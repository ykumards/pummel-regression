...
class EHRDataset(Dataset):
    ...
    def load_dataset_and_make_vectorizer(cls, df):
        ...
        return cls(df, 
                   EHRCountVectorizer.from_dataframe(train_df))
    ...
    def __getitem__(self, index):
        ...
        diag_for_patient, patient_num_visits = \
            self.vectorizer.vectorize(row.X_seq)
...
# class EHRMultiHotDataset(Dataset):
#     def __init__(self, 
#                  df, 
#                  vectorizer, target_vectorizer,
#                  pal_vectorizer, yh_vectorizer, 
#                  include_features=True, 
#                  include_timedels=True, 
#                  num_workers=8):
#         self.df = df
...
#         self.pal_vectorizer = pal_vectorizer
#         self.yh_vectorizer = yh_vectorizer
#         self.include_features = include_features
#         self.include_timedels = include_timedels
#         self.num_workers = num_workers
...
#         train_df = df[df.split == 'train']
#         return cls(df, 
#                    EHRMultiHotVectorizer.from_dataframe(train_df),
#                    EHRMultiHotVectorizer.from_dataframe(train_df, 'Y_seq'),
#                    EHRMultiHotVectorizer.from_dataframe(train_df, 'pal'), 
#                    EHRMultiHotVectorizer.from_dataframe(train_df, 'yh'))
...
#         diag_for_patient, patient_num_visits = \
#             self.vectorizer.vectorize(row.X_seq)
#         target_y, _ = self.target_vectorizer.vectorize(row.Y_seq)
#         yh, _ = self.yh_vectorizer.vectorize(row.yh)
#         pal, _ = self.pal_vectorizer.vectorize(row.pal)
#         # target_y = row.target_y
#         gender, age, timedels = None, None, []
