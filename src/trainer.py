...
class Trainer(object):
    ...
    def pad_x_sequence_along_diagnoses(
        ...
        for i, visit in enumerate(seq):
            padded_vector_along_diagnoses[i, :len(visit)] = visit # pads zeros to the front
    ...
    def collate_fn(self, batch):
        ...
        for _, sample in enumerate(batch):
            padded_x_seq_along_diagnoses = self.pad_x_sequence_along_diagnoses(
                sample["x_seq"],
                self.args.max_diag_length_per_visit
            )
        ...
        processed_batch["seq_length"] = torch.LongTensor(
            processed_batch["seq_length"]
        )
        processed_batch["target_y"] = torch.LongTensor(processed_batch["target_y"]).squeeze(0)
        ...
        if self.args.include_features:
            processed_batch["age"] = torch.FloatTensor(
                processed_batch["age"]).unsqueeze(1)
            processed_batch["prev_num_visits"] = torch.FloatTensor(
                processed_batch["prev_num_visits"]).unsqueeze(1)
            processed_batch["time_delta_mean"] = torch.FloatTensor(
                processed_batch["time_delta_mean"]).unsqueeze(1)
    ...
    def update_train_state(self, verbose=True):
        ...
        if self.writer is not None:
            ...
            self.writer.add_histogram('validation y_preds', np.array(self.train_state['val_preds']), ep)
            self.writer.add_histogram('validation y_true', np.array(self.train_state['val_true']), ep)

        if self.train_state['epoch_index'] == 0:
            torch.save(self.model.state_dict(),
                       self.train_state['model_filename'])
            torch.save(self.optimizer.state_dict(), 
                       self.train_state['optim_filename'])
            ...
            if (self.train_state['epoch_index'] == 5) or (self.train_state['reduce_lr_step'] >= self.train_state['reduce_lr_criteria']):
        ...
        # Verbose
        if verbose:
            print(f"[{ep}]: | lr: {print_lr} | tr_loss: {tr_loss:.4f} | " + \
    ...
    def _get_model_addln_args(self, batch_dict):
        ...
        if self.args.include_features:
            model_addln_args['x_time_delta_mean'] = batch_dict['time_delta_mean'].to(self.args.device)
    ...
    def train(self, verbose=True):
        ...
        for epoch in range(self.args.num_epochs):
            ...
            for batch_idx, batch_dict in enumerate(tqdm(batch_generator, 
                                                        total=self.dataset.get_num_batches(
                ...
                loss = self.loss_func(
                    y_pred,
                    batch_dict['target_y'].to(self.args.device).float()
                )
    ...
    def heldout(self):
        ...
        for batch_dict in tqdm(batch_generator, 
            ...
            y_pred = self.model(batch_dict['x_seq'].to(self.args.device), 
                                batch_dict['seq_length'].to(self.args.device),
                                **model_addln_args)            

            # compute the loss
            loss = self.loss_func(y_pred,
                                  batch_dict['target_y'].to(self.args.device).float())
            ...
            self.train_state['heldout_true_preds_id_tuple'] += list(zip(batch_dict['krypht'], 
                                                                        y_pred.tolist(),
        ...
        if self.writer is not None:
            self.writer.add_histogram('ho y_preds', np.array(self.train_state['heldout_preds']), 0)
            self.writer.add_histogram('ho y_true', np.array(self.train_state['heldout_true']), 0)
    ...
    def plot_residuals(self, filename):
        ...
        ax1.scatter(ytrue, ypred, s=10, c='r', alpha=0.3,
                    marker="o", label='predicted', cmap='viridis')
        plt.xlabel('ytrue')
        plt.ylabel('ypred')
