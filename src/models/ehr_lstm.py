...
class EHR_LSTM(nn.Module):
    ...
    def forward(
        ...
        x_gender = self.gender_embed(x_gender)\
                       .unsqueeze(1).expand(batch_size, max_seq_length, -1)
        x_age = x_age.unsqueeze(1)\
                     .expand(batch_size, max_seq_length, -1)
        x_time_delta_mean = x_time_delta_mean\
                            .unsqueeze(1).expand(batch_size, max_seq_length, -1)
        ...
        rnn_attn_max = F.adaptive_max_pool1d(
            attn_out.permute(0, 2, 1), 1).squeeze()
        
        rnn_out = gather_last_relevant_hidden(rnn_outs, x_lengths)

        if self.use_mh_attn:
            rnn_out = (1 - F.sigmoid(self.attn_alpha)) * rnn_out +  rnn_attn_max * F.sigmoid(self.attn_alpha)
